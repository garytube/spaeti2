...
<div
  ...
  {#if $product.length > 0}
    <div
      class="h-4 w-4 animate-pulse rounded-full absolute -top-1 -right-1 text-[12px] bg-pink-600 font-body ring-2 ring-primary text-white flex justify-center items-center z-[1000]">
      {$product.length}
    </div>
  {/if}
